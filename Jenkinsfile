        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
                // run sonar scanner
                sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.python.xunit.skipDetails=true -Dsonar.sources=./insitu -Dsonar.exclusions=**/tests/**,**/setup.py  -Dsonar.projectKey=\"${GIT_NAME}\" -Dsonar.projectName=\"${GIT_NAME}\" -Dsonar.branch.name=\"${BRANCH_NAME}\""

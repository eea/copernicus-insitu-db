...
class ReportExcelMixin:
    ...
    def generate_table_1(self, workbook, worksheet):
        ...
        requirements = Requirement.objects.filter(products__in=self.products).distinct()
        index = 2
    ...
    def generate_table_3(self, workbook, worksheet):
        ...
        worksheet.set_row(0, 30)
        worksheet.set_row(1, 17)
        worksheet.merge_range('A1:C1', 'PRODUCTS AND ASSOCIATED REQUIREMENTS', self.format_header)
        headers = [
        ...
        index = 2
        for product in self.products:
            requirement_count = product.product_requirements.all().count()
            if requirement_count >= 2:
                worksheet.merge_range(index, 0, index + requirement_count - 1, 0, product.name, self.format_rows)
                for product_requirement in product.product_requirements.all():
                    data = [product_requirement.requirement.name, product_requirement.requirement.id]
                    worksheet.write_row(index, 1 , data, self.format_rows)
                    index += 1
            elif requirement_count == 1:
                requirement = product.product_requirements.first().requirement
                worksheet.write_row(index, 0 , [product.name, requirement.name, requirement.id], self.format_rows)
                index += 1
            else:
    ...
    def generate_table_4(self, workbook, worksheet):
        ...
        worksheet.set_row(0, 30)
        worksheet.set_row(1, 17)
        worksheet.merge_range('A1:E1', 'DATASETS AND RELATED DATA PROVIDERS PER REQUIREMENT', self.format_header)
        headers = [
        ...
        for requirement in requirements:
            ...
            for datarequirement in requirement.datarequirement_set.all():
                ...
                for data_provider in datarequirement.data.providers.all():
                    worksheet.write_row(provider_index, 3, [data_provider.name, getattr(getattr(data_provider.details.first(), 'provider_type', ''), 'name', '')], self.format_rows)
                    provider_index += 1
                if provider_index == data_index:
                    worksheet.write_row(data_index, 2, [datarequirement.data.name, '', ''], self.format_rows)
                    data_index = provider_index
                elif provider_index == data_index + 1:
                    worksheet.write_row(data_index, 2, [datarequirement.data.name], self.format_rows)
                else:
                    worksheet.merge_range(data_index, 2, provider_index -1, 2, datarequirement.data.name, self.format_rows)
                data_index = provider_index
            if data_index == requirement_index:
                worksheet.write_row(requirement_index, 0, [requirement.name, requirement.id, '', '', ''], self.format_rows)
                requirement_index = data_index + 1
            elif data_index == requirement_index + 1:
                worksheet.write_row(requirement_index, 0, [requirement.name, requirement.id], self.format_rows)
                requirement_index = data_index
            else:
                worksheet.merge_range(requirement_index, 0, data_index - 1, 0, requirement.name, self.format_rows)
                worksheet.merge_range(requirement_index, 1, data_index - 1, 1, requirement.id, self.format_rows)
                requirement_index = data_index
    ...
    def generate_table_5(self, workbook, worksheet):
        ...
        worksheet.set_row(0, 30)
        worksheet.set_row(1, 17)
        worksheet.merge_range('A1:F1', 'MAIN DETAILS BETWEEN REQUIREMENTS AND PRODUCTS', self.format_header)
        headers = [
        ...
        for product in self.products:
            ...
            if product_merge_dimension >= 2:
                ...
                worksheet.write_row(index, 0 , [product.name, product_requirement.requirement.name, product_requirement.requirement.id,
                                                "\n".join([x.name for x in product_requirement.barriers.all()]), product_requirement.relevance.name, product_requirement.criticality.name], self.format_rows)
                index += 1
            else:
                worksheet.write_row(index, 0 , [product.name, '', '', '', '', ''], self.format_rows)
                index += 1
    ...
    def generate_table_6(self, workbook, worksheet):
        ...
        worksheet.set_row(0, 30)
        worksheet.set_row(1, 17)
        worksheet.merge_range('A1:E1', 'LEVEL OF COMPLIANCE BETWEEN DATASET AND REQUIREMENT', self.format_header)
        headers = [
        ...
        for product in self.products:
            ...
            data = Data.objects.filter(requirements__in=requirements, datarequirement___deleted=False).distinct()
            data_index = product_index
            for data_object in data:
                data_requirement_index = data_index
                for data_requirement in data_object.datarequirement_set.all():
                    worksheet.write_row(data_requirement_index, 2, [data_requirement.requirement.name, data_requirement.requirement.id, data_requirement.note])
                    data_requirement_index += 1
                if data_index == data_requirement_index:
                    worksheet.write_row(data_index, 1, [data_object.name, '', '', ''], self.format_rows)
                    data_index = data_requirement_index
                elif data_requirement_index == data_index + 1:
                    worksheet.write_row(data_index, 1, [data_object.name], self.format_rows)
                else:
                    worksheet.merge_range(data_index, 1, data_requirement_index - 1, 1, data_object.name, self.format_rows)
                data_index = data_requirement_index
            if data_index == product_index:
                ...
                worksheet.merge_range(product_index, 0, data_index - 1, 0, product.name, self.format_rows)
                product_index = data_index
    ...
    def generate_table_8(self, workbook, worksheet):
        ...
        worksheet.set_row(0, 30)
        worksheet.set_row(1, 17)
        worksheet.merge_range('A1:F1', 'DATASETS AND RELATED DATA PROVIDERS', self.format_header)
        headers = [
        ...
        for data_object in data:
            ...
            if provider_count >= 2:
                worksheet.merge_range(index, 0, index + provider_count - 1, 0, data_object.name, self.format_rows)
                for dataprovider in data_object.providers.all():
                    row_data = [
                        dataprovider.name,
                        getattr(getattr(dataprovider.details.first(), 'provider_type', ''), 'name', ''),
                        getattr(data_object.quality_control_procedure, 'name', ''),
                        getattr(data_object.dissemination, 'name', ''), getattr(data_object.timeliness, 'name', '')
                    ]
                ...
                worksheet.write_row(index, 0 , [data_object.name, '', '', '', '', ''], self.format_rows)
                index += 1
